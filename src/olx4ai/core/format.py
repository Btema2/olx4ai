...
def print_stats(rows: list[dict], label: str) -> None:
    ...
    if stats["priced_count"] == 0:
        return
    print(
        f"  min {stats['min']}  p25 {stats['p25']}  median {stats['median']}  "
...
def emit(rows: list[dict], args, label: str) -> None:
    ...
    if not rows:
        ...
        return
    print(
        f"# {label} — {len(rows)} offers   (flags: ~negotiable D=delivery "
    ...
    if prices:
        print(
            f"# price: min {min(prices)} / median {int(statistics.median(prices))} "
            f"/ max {max(prices)} zł"
        )
